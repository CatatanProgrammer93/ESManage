...
namespace es_manage.api.Repositories
{
    public class RoleRepository
    {
        ...
        public async Task<IEnumerable<RoleModel>> RoleSearch(string search, int limit, int page)
        {
            try
            {
                int offset = 0;
                var sql = $"SELECT * FROM Role WHERE (Id iLIKE @Search OR RoleName iLIKE @Search OR CreatedBy iLIKE @Search) AND Deleted = FALSE LIMIT @Limit";
                search = '%' + search + '%';
                if(page > 1){
                    offset = (page - 1) * limit;
                    sql = sql + " OFFSET @Offset";
                }
                return await _db.QueryAsync<RoleModel>(sql, new { Search = search, Limit = limit, Offset = offset});
            }
            catch (Exception ex)
            {
                Logger.WriteToConsole(Logger.LogType.Error, ex.Message);
