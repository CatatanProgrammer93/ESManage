...
namespace es_manage.api.Repositories {
    public class BrandRepository {
        ...
        public async Task<IEnumerable<BrandModel>> BrandSearch(string search, int limit, int page)
        {
            try
            {
                int offset = 0;
                var sql = $"SELECT * FROM Brand WHERE (Id iLIKE @Search OR Name iLIKE @Search OR CreatedBy iLIKE @Search) AND Deleted = FALSE LIMIT @Limit";
                search = '%' + search + '%';
                if(page > 1){
                    offset = (page - 1) * limit;
                    sql = sql + " OFFSET @Offset";
                }
                return await _db.QueryAsync<BrandModel>(sql, new {Search = search, Limit = limit, Offset = offset});
            }
            catch (Exception ex)
            {
                Logger.WriteToConsole(Logger.LogType.Error, ex.Message);
