...
namespace es_manage.api.Repositories {
    public class ItemRepository {
        ...
        public async Task<IEnumerable<ItemModel>> ItemSearch(string search, int limit, int page)
        {
            try
            {
                int offset = 0;
                var sql = $"SELECT * FROM Item WHERE (Id iLIKE @Search OR ItemName iLIKE @Search OR CategoryId iLIKE @Search OR CategoryName iLIKE @Search OR BrandId iLIKE @Search OR CreatedBy iLIKE @Search) AND Deleted = FALSE LIMIT @Limit";
                search = '%' + search + '%';
                if(page > 1){
                    offset = (page - 1) * limit;
                    sql = sql + " OFFSET @Offset";
                }
                return await _db.QueryAsync<ItemModel>(sql, new { Search = search, Limit = limit, Offset = offset});
            }
            catch (Exception ex)
            {
                Logger.WriteToConsole(Logger.LogType.Error, ex.Message);
